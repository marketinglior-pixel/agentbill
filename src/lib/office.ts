...
export function shareText(o: Office, sample: boolean): string {
  const pay = o.payroll == null ? null : `$${(Math.round(o.payroll * 100) / 100).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
  const lead = sample ? 'A sample payroll for AI agents' : 'Payroll for my AI agents this month'
  const sent = o.sentHome ? `, ${o.sentHome} sent home by a spend ceiling` : ''
  return `${lead}: ${pay ?? 'unpriced'} across ${o.staff} ${o.staff === 1 ? 'agent' : 'agents'}${sent}. Made with AgentBill, agentbill.dev`
}
